...
\subsection{Results}
	...
	\paragraph{The monolithic code structure} The final form of the system can be described as complex if not complicated. I tried my best to separate my code into a \textbf{Model View Control} pattern. The model represents the audio synthesis structure of the firmware: the oscillators, voices, filters, parameter structure and the synth class the contains them. It represents the core functionality of the system. The view represents the components that convey the state of the system to the user: the LCD screen, the menus and their submenus. The control represents the way that the user interacts with the system: all the physical knobs, inputs, hardware interupts, etc. Keeping these parts of the MVC separate was difficult. In the end, some structure needed to own them. Hardware needed to receive inputs and communicate them to the Model and View. So in the end, the hardware layer, the Control in MVC, became the sole owner of everything.
